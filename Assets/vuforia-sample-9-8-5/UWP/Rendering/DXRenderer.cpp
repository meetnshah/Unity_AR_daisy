...
namespace winrt::VuforiaSample::implementation
{
    ...
    void DXRenderer::CreateDeviceDependentResources()
    {
        ...
        auto createConstColorVSTask = loadConstColorVSTask.then([this](const std::vector<byte>& fileData)
        {
            initConstColorVertexShader(fileData);
        });
        auto createVertexColorVSTask = loadVertexColorVSTask.then([this](const std::vector<byte>& fileData)
        {
            initVertexColorVertexShader(fileData);
        });
        auto createTexturedVSTask = loadTexturedVSTask.then([this](const std::vector<byte>& fileData)
        {
            initTexturedVertexShader(fileData);
        });
        auto createVideoBgVSTask = loadVideoBgVSTask.then([this](const std::vector<byte>& fileData)
        {
            initVideoBackgroundVertexShader(fileData);
        });

        // After the pixel shader file is loaded, create the shader and constant buffer.
        auto createConstColorPSTask = loadConstColorPSTask.then([this](const std::vector<byte>& fileData)
        {
            initConstColorPixelShader(fileData);
        });
        auto createVertexColorPSTask = loadVertexColorPSTask.then([this](const std::vector<byte>& fileData)
        {
            initVertexColorPixelShader(fileData);
        });
        auto createTexturedPSTask = loadTexturedPSTask.then([this](const std::vector<byte>& fileData)
        {
            initTexturedPixelShader(fileData);
        });
        auto createVideoBgPSTask = loadVideoBgPSTask.then([this](const std::vector<byte>& fileData)
        {
            initVideoBackgroundPixelShader(fileData);
        });

        // Load shaders in parallel then setup object-specific rendering
        auto createAugmentationModelsTask = (
            createConstColorPSTask && createConstColorVSTask && 
            createVertexColorPSTask && createVertexColorVSTask &&
            createTexturedPSTask && createTexturedVSTask &&
            createVideoBgPSTask && createVideoBgVSTask).then([this]()
        {
            initSquare();
            initCube();
            initAxis();
        ...
        auto setupRasterizersTask = createTextureTask.then([this]()
        {
            ...
            D3D11_RASTERIZER_DESC augmentationRasterDescCullBack = DX::CreateRasterizerDesc(
            ...
            D3D11_DEPTH_STENCIL_DESC augmentDepthStencilDesc = DX::CreateDepthStencilDesc(
                true,
    ...
    void DXRenderer::ReleaseDeviceDependentResources()
    {
        mRendererInitialized = false;
    ...
    void DXRenderer::initVideoBackgroundTexture(size_t width, size_t height)
    {
        ...
        // Create the texture
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateTexture2D(&texDesc, nullptr, mVBTexture.put())
            );
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateShaderResourceView(
                mVBTexture.get(), &SRVDesc, mVBTextureView.put()
                )
            );
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateSamplerState(
                &samplerDesc, mVBSamplerState.put()
                )
            );
    ...
    void DXRenderer::renderVideoBackground(Vuforia::Matrix44F& projectionMatrix,
        const int numVertices,
        const Vuforia::Vec3F* vbVertices, const Vuforia::Vec2F* vbTexCoords,
        const int numTriangles, const unsigned short* indices)
    {
        ...
        if (!mVideoBackgroundMeshInitialized)
        {
            ...
            initVideoBackgroundMesh(numVertices, vbVertices, vbTexCoords, numTriangles, indices, device);
        }
    ...
    void DXRenderer::prepareForAugmentationRendering()
    {
        auto context = mDeviceResources->GetD3DDeviceContext();
    ...
    void DXRenderer::renderWorldOrigin(
        ...
        Vuforia::Matrix44F& modelViewMatrix)
    {
        ...
        Vuforia::Vec3F axisScaleVec(WORLDORIGIN_AXES_SCALE, WORLDORIGIN_AXES_SCALE, WORLDORIGIN_AXES_SCALE);
        scaledModelViewMatrix = MathUtils::Matrix44FScale(axisScaleVec, modelViewMatrix);
        DirectX::XMMATRIX axisModelViewDX = convertVuforiaMatrixToDX(scaledModelViewMatrix);

        Vuforia::Vec3F cubeScaleVec(WORLDORIGIN_CUBE_SCALE, WORLDORIGIN_CUBE_SCALE, WORLDORIGIN_CUBE_SCALE);
        scaledModelViewMatrix = MathUtils::Matrix44FScale(cubeScaleVec, modelViewMatrix);
        ...
        renderConstColor(projectionMatrixDX, cubeModelViewDX, color, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST,
            mCubeVertexBuffer, mCubeSolidIndexBuffer, NUM_CUBE_INDEX);
    }


    void DXRenderer::renderImageTarget(Vuforia::Matrix44F& projectionMatrix,
        Vuforia::Matrix44F& modelViewMatrix, Vuforia::Matrix44F& scaledModelViewMatrix)
    {
        ...
        renderConstColor(projectionMatrixDX, boundingBoxModelViewDX, solidColor, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST,
            mSquareVertexBuffer, mSquareSolidIndexBuffer, NUM_SQUARE_INDEX);

        DirectX::XMVECTOR wireframeColor = DirectX::Colors::Red;
        renderConstColor(projectionMatrixDX, boundingBoxModelViewDX, wireframeColor, D3D11_PRIMITIVE_TOPOLOGY_LINELIST,
            mSquareVertexBuffer, mSquareWireframeIndexBuffer, NUM_SQUARE_WIREFRAME_INDEX);
    ...
    void DXRenderer::renderModelTarget(Vuforia::Matrix44F& projectionMatrix,
        Vuforia::Matrix44F& modelViewMatrix, Vuforia::Matrix44F& /*scaledModelViewMatrix*/)
    {
        DirectX::XMMATRIX projectionMatrixDX = convertVuforiaMatrixToDX(projectionMatrix);
    ...
    void DXRenderer::renderModelTargetGuideView(Vuforia::Matrix44F& projectionMatrix,
        Vuforia::Matrix44F& modelViewMatrix,
    ...
    void DXRenderer::initConstColorVertexShader(const std::vector<byte>& fileData)
    {
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateVertexShader(
                ...
                fileData.size(),
                nullptr,
                mConstColorVertexShader.put()
                )
            );

        CD3D11_BUFFER_DESC constantBufferDesc(
            sizeof(SampleCommon::ConstColorShaderConstantBuffer),
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateInputLayout(
                ...
                &fileData[0],
                fileData.size(),
                mConstColorInputLayout.put()
                )
            );
    }


    void DXRenderer::initConstColorPixelShader(const std::vector<byte>& fileData)
    {
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreatePixelShader(
                ...
                fileData.size(),
                nullptr,
                mConstColorPixelShader.put()
                )
            );
    }


    void DXRenderer::initVertexColorVertexShader(const std::vector<byte>& fileData)
    {
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateVertexShader(
                ...
                fileData.size(),
                nullptr,
                mVertexColorVertexShader.put()
                )
            );

        CD3D11_BUFFER_DESC constantBufferDesc(
            sizeof(SampleCommon::VertexColorShaderConstantBuffer),
            D3D11_BIND_CONSTANT_BUFFER);
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateBuffer(
                &constantBufferDesc,
                nullptr,
                mVertexColorConstantBuffer.put()
                )
            );
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateInputLayout(
                ...
                &fileData[0],
                fileData.size(),
                mVertexColorInputLayout.put()
                )
            );
    }


    void DXRenderer::initVertexColorPixelShader(const std::vector<byte>& fileData)
    {
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreatePixelShader(
                ...
                fileData.size(),
                nullptr,
                mVertexColorPixelShader.put()
                )
            );
    }


    void DXRenderer::initTexturedVertexShader(const std::vector<byte>& fileData)
    {
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateVertexShader(
                &fileData[0],
                fileData.size(),
                nullptr,
                mTexturedVertexShader.put()
                )
            );

        CD3D11_BUFFER_DESC constantBufferDesc(
            sizeof(SampleCommon::TexturedShaderConstantBuffer),
            D3D11_BIND_CONSTANT_BUFFER);
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateInputLayout(
                ...
                &fileData[0],
                fileData.size(),
                mTexturedInputLayout.put()
                )
            );
    }
    
    
    void DXRenderer::initTexturedPixelShader(const std::vector<byte>& fileData)
    {
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreatePixelShader(
                &fileData[0],
                fileData.size(),
                nullptr,
                mTexturedPixelShader.put()
                )
            );
    }


    void DXRenderer::initVideoBackgroundVertexShader(const std::vector<byte>& fileData)
    {
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateVertexShader(
                ...
                fileData.size(),
                nullptr,
                m_vbVertexShader.put()
                )
            );

        CD3D11_BUFFER_DESC constantBufferDesc(
            sizeof(SampleCommon::VideoBackgroundShaderConstantBuffer),
            D3D11_BIND_CONSTANT_BUFFER);
    ...
    void DXRenderer::initVideoBackgroundPixelShader(const std::vector<byte>& fileData)
    {
    ...
    void DXRenderer::initVideoBackgroundRenderState()
    {
        ...
        D3D11_RASTERIZER_DESC videoRasterDescCounterClockwise = DX::CreateRasterizerDesc(
        ...
        D3D11_DEPTH_STENCIL_DESC videoDepthStencilDesc = DX::CreateDepthStencilDesc(
            false,
    ...
    void DXRenderer::initVideoBackgroundMesh(const int numVertices,
        const Vuforia::Vec3F* vbVertices, const Vuforia::Vec2F* vbTexCoords,
        const int numTriangles, const unsigned short* vbIndices, ID3D11Device* device)
    {
        ...
        {
            vbMeshVertices[i].pos = DirectX::XMFLOAT3(
                vbVertices[i].data[0],
                vbVertices[i].data[1],
                vbVertices[i].data[2]
                );

            vbMeshVertices[i].texcoord = DirectX::XMFLOAT2(
    ...
    void DXRenderer::initSquare()
    {
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateBuffer(
                &wireframeIndexBufferDesc,
                &wireframeIndexBufferData,
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateBuffer(
                &solidIndexBufferDesc,
                &solidIndexBufferData,
                mSquareSolidIndexBuffer.put()
                )
            );
    }

    
    void DXRenderer::initCube()
    {
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateBuffer(
                &solidIndexBufferDesc,
                &solidIndexBufferData,
                mCubeSolidIndexBuffer.put()
                )
            );
    }


    void DXRenderer::initAxis()
    {
        LOG("initAxis");
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateBuffer(
                &indexBufferDesc,
                &indexBufferData,
                mAxesIndexBuffer.put()
                )
            );
    }


    void DXRenderer::initGuideView()
    {
        ...
        SampleCommon::TexturedShaderInputBuffer meshVertices[NUM_GUIDE_VIEW_VERTEX] = {
        ...
        winrt::check_hresult(
            mDeviceResources->GetD3DDevice()->CreateBuffer(
                &vertexBufferDesc,
                &vertexBufferData,
                mGuideViewVertexBuffer.put()
                )
            );
    }


    void DXRenderer::initModels()
    {
        LOG("initModels");
    ...
    int DXRenderer::initBuffersFromModel(
        tinyobj::attrib_t& attrib,
        ...
        for (size_t s = 0, i = 0; s < shapes.size(); ++s)
        {
            ...
            for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); ++f)
            {
                ...
                for (size_t v = 0; v < size_t(fv); ++v, ++i)
                {
                    ...
                    inputBufferRaw[i].pos = DirectX::XMFLOAT3(
                        attrib.vertices[3L * idx.vertex_index + 0],
                    ...
                    {
                        inputBufferRaw[i].texcoord = DirectX::XMFLOAT2(
                            attrib.texcoords[2L * idx.texcoord_index + 0],
    ...
    DirectX::XMMATRIX DXRenderer::convertVuforiaMatrixToDX(const Vuforia::Matrix44F& vuforiaMatrix)
    {
    ...
    void DXRenderer::renderConstColor(
        const DirectX::XMMATRIX& projection,
        const DirectX::XMMATRIX& modelView,
        DirectX::XMVECTOR& color,
        D3D_PRIMITIVE_TOPOLOGY topology,
        winrt::com_ptr<ID3D11Buffer> verticies,
        winrt::com_ptr<ID3D11Buffer> indicies,
        uint32_t indexCount)
    {
        auto context = mDeviceResources->GetD3DDeviceContext();
    ...
    void DXRenderer::renderAxis(const Vuforia::Matrix44F& projectionMatrix,
                              ...
                              const Vuforia::Vec3F& scale)
    {
        ...
        // Set model-view matrix
        auto adjustedModelView = MathUtils::Matrix44FScale(scale, modelViewMatrix);
        ...
        // Prepare the constant buffer to send it to the graphics device.
        context->UpdateSubresource1(
            mVertexColorConstantBuffer.get(),
            0,
            NULL,
            &constantBufferData,
            0,
    ...
    void DXRenderer::renderModel(
        const DirectX::XMMATRIX& projection, const DirectX::XMMATRIX& modelView,
        winrt::com_ptr<ID3D11Buffer> vertices, int numVertices,
        SampleCommon::Texture* texture)
    {
